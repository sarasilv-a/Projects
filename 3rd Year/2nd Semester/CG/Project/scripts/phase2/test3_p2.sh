...
generate_model() {
    ...
    if [[ ! -f "../$MODELS/$filename" ]]; then
        echo "Erro: modelo $filename não gerado!"
        exit 1
    fi
}
...
# Executar o engine
echo "Executando engine para test_2_3.xml..."
./engine "../$SCENES/tests/test_files_phase_2/test_2_3.xml"
cd - || exit 1

echo "Teste 2_3 finalizado com sucesso!"
