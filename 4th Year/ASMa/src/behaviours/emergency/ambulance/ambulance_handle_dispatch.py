...
class AmbulanceHandleDispatchBehaviour(CyclicBehaviour):
    async def run(self):
        ...
        # Só processa se for ambulância
        if payload.get("transport_type") != TransportType.AMBULANCE.value:
            return
        ...
        if (not self.agent.available) and (emergency_id == self.agent.current_emergency_id):
            ...
            logger.info(f"[AMBULANCE] REROUTE emergency_id={emergency_id} -> hospital={hospital_jid}")
            return

        if not self.agent.available:
            refuse_payload = {
                "emergency_id": emergency_id,
                "reason": "ambulance_busy",
                "timestamp": get_current_time(),
            }
            ...
            logger.warning(f"[AMBULANCE] REFUSE transport_dispatch: {refuse_payload}")
            return
