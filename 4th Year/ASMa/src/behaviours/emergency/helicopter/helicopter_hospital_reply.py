...
class HelicopterHospitalReplyBehaviour(CyclicBehaviour):
    async def run(self):
        ...
        logger.info(f"[HELICOPTER] Hospital reply perf={perf} type={mtype} payload={payload}")

        # Propaga admitted/refused ao CENTER (para o CENTER fechar / reencaminhar)
        ...
        if mtype == EmergencyMsg.PATIENT_ADMITTED.value:
            ...
            # hospital_full -> mantém-se ocupado à espera de reroute do CENTER
            if payload.get("status") == "hospital_full":
                return

            # recusa final: libertar
            done_payload = {
                "emergency_id": emergency_id,
                "transport_type": "helicopter",
                "timestamp": get_current_time(),
